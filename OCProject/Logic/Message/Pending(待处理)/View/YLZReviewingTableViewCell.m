...
@interface YLZReviewingTableViewCell()
...
@implementation YLZReviewingTableViewCell
...
- (void)setModel:(YLZReviewingModel *)model {
    _model = model;
    ...
    if (model.isPass) {
        ...
        [self.notPassButton setTitle:@"通过" forState:UIControlStateNormal];
        ...
        [self.notPassButton setTitle:@"不通过" forState:UIControlStateNormal];
        self.notPassButton.hidden = NO;
...
- (UIButton *)linkButton {
    if (!_linkButton) {
        ...
        _linkButton.tag = 0;
        [_linkButton setTitle:@"查看用户主页 " forState:UIControlStateNormal];
        [_linkButton setImage:[UIImage imageNamed:@"ylz_arrow_orange"] forState:UIControlStateNormal];
...
- (UIButton *)notPassButton {
    if (!_notPassButton) {
        ...
        [_notPassButton setTitle:@"不通过" forState:UIControlStateNormal];
