...
- (void)setup {
    self.numberOfItems = 40.f;//偶数

    self.itemColor = [UIColor colorWithHexString:@"#FF7700"];
...
- (void)updateItems {
    ...
    for(int i = 0; i < self.numberOfItems / 2; i++) {
        ...
        NSInteger arc4random1 = arc4random() % 10 +6;  // 0 - 9的随机数

        CGFloat edx = [self.levels[i] floatValue];
        
        edx = edx * arc4random1 / 10.00;
        
        CGFloat lineHeight = self.lineWidth + edx * self.lineWidth / 2.f;
...
//        [linePathLeft addLineToPoint:CGPointMake(leftX, lineBottom)];
        // 创建有圆角的
        UIBezierPath *linePathLeft = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(leftX +4, lineTop, self.lineWidth- 4, lineBottom - lineTop) cornerRadius:1.5];
        ...
        NSInteger arc4random2 = arc4random() % 10 +6;  // 0 - 9的随机数
        edx = edx * arc4random2 / 10.00;
        lineHeight = self.lineWidth + edx * self.lineWidth / 2.f;
        lineTop = (self.itemHeight - lineHeight) / 2.f ;
        lineBottom = (self.itemHeight + lineHeight) / 2.f ;
...
//        [linePathRight addLineToPoint:CGPointMake(rightX, lineBottom)];
        // 创建有圆角的
        UIBezierPath *linePathRight = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(rightX, lineTop, self.lineWidth- 4, lineBottom - lineTop) cornerRadius:1.5];
