...
#import "HSAQRCodeObtain.h"
#import "HSAQRCodeObtainConfigure.h"
#import <Photos/Photos.h>

@interface HSAQRCodeObtain () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>
@property (nonatomic, weak) UIViewController *controller;
@property (nonatomic, strong) HSAQRCodeObtainConfigure *configure;
@property (nonatomic, strong) AVCaptureSession *captureSession;
@property (nonatomic, copy) HSAQRCodeObtainScanResultBlock scanResultBlock;
@property (nonatomic, copy) HSAQRCodeObtainScanBrightnessBlock scanBrightnessBlock;
@property (nonatomic, copy) HSAQRCodeObtainAlbumDidCancelImagePickerControllerBlock albumDidCancelImagePickerControllerBlock;
@property (nonatomic, copy) HSAQRCodeObtainAlbumResultBlock albumResultBlock;
...
@implementation HSAQRCodeObtain
...
- (void)establishQRCodeObtainScanWithController:(UIViewController *)controller configure:(HSAQRCodeObtainConfigure *)configure {
    if (controller == nil) {
        @throw [NSException exceptionWithName:@"HSAQRCode" reason:@"HSAQRCodeObtain 中 establishQRCodeObtainScanWithController:configuration:方法的 controller 参数不能为空" userInfo:nil];
    }
    if (configure == nil) {
        @throw [NSException exceptionWithName:@"HSAQRCode" reason:@"HSAQRCodeObtain 中 establishQRCodeObtainScanWithController:configure:方法的 configure 参数不能为空" userInfo:nil];
    }
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputMetadataObjects:(NSArray<__kindof AVMetadataObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    [metadataObjects enumerateObjectsUsingBlock:^(__kindof AVMetadataObject * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        AVMetadataObject *transformedCode = (AVMetadataMachineReadableCodeObject*)[weak_self.videoPreviewLayer transformedMetadataObjectForMetadataObject:metadataObjects.lastObject];
        ...
        {
            [weak_self videoZoomFactor: 150 / size animation:YES];
            *stop = YES;
            return;
        }
        ...
        if ([transformedCode isKindOfClass:[AVMetadataMachineReadableCodeObject class]])
        {
            ...
            if (code.stringValue)
            {
                if (self.scanResultBlock) {
                    self.scanResultBlock(self, [code stringValue]);
                }
            }
            *stop = YES;
            return;
...
- (void)videoZoomFactor:(CGFloat)zoomValue animation:(BOOL)animation {
    AVCaptureDevice *device = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo];
...
- (void)setBlockWithQRCodeObtainScanResult:(HSAQRCodeObtainScanResultBlock)block {
    _scanResultBlock = block;
}
- (void)setBlockWithQRCodeObtainScanBrightness:(HSAQRCodeObtainScanBrightnessBlock)block {
...
- (void)playSoundName:(NSString *)name {
    ...
    NSLog(@"playSoundName__________%@",path);
...
- (void)establishAuthorizationQRCodeObtainAlbumWithController:(UIViewController *)controller {
    if (controller == nil && _controller == nil) {
        @throw [NSException exceptionWithName:@"HSAQRCode" reason:@"HSAQRCodeObtain 中 establishAuthorizationQRCodeObtainAlbumWithController: 方法的 controller 参数不能为空" userInfo:nil];
    }
    if (_controller == nil) {
        _controller = controller;
    }
...
- (void)setBlockWithQRCodeObtainAlbumDidCancelImagePickerController:(HSAQRCodeObtainAlbumDidCancelImagePickerControllerBlock)block {
    _albumDidCancelImagePickerControllerBlock = block;
}
- (void)setBlockWithQRCodeObtainAlbumResult:(HSAQRCodeObtainAlbumResultBlock)block {
