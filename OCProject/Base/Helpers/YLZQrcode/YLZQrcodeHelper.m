...
#import "YLZQrcodeHelper.h"

@implementation YLZQrcodeHelper
 ...
 * @param logoScaleToSuperView 相对于父视图的缩放比（取值范围：0-1，0，代表不显示，1，代表与父视图大小相同）
 * @return 返回二维码图片
 */
...
+ (UIImage *)generateWithColorQRCodeData:(NSString *)data backgroundColor:(CIColor *)backgroundColor mainColor:(CIColor *)mainColor {
    ...
    // 图片小于(27,27),我们需要放大
    outputImage = [outputImage imageByApplyingTransform:CGAffineTransformMakeScale(1, 1)];
...
#pragma mark - 获取条形码
+ (UIImage*)generateWithBarCodeData:(NSString*)str
{
    return [self generateWithBarCodeData:str scale:1];
}

+ (UIImage*)generateWithBarCodeData:(NSString*)str scale:(CGFloat)scale
{
    // 将字符串转换成NSData
...
+ (UIImage*)generateWithBarCodeData:(NSString*)str imageViewWidth:(CGFloat)imageViewWidth
{
    // 将字符串转换成NSData
    NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
