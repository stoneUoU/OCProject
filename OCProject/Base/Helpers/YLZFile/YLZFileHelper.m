...
#import "YLZFileHelper.h"

@implementation YLZFileHelper
...
+ (YLZFile *)scanRelativeFilePath:(NSString *)relativeFilePath maxTreeLevel:(NSInteger)maxTreeLevel {
    ...
    NSString *filePath = [YLZFileHelper absoluteFilePathFromRelativeFilePath:relativeFilePath];
    ...
    if (isExist) {
        ...
        file = [YLZFileHelper cerateFileWithFilePath:filePath isDirectory:isDirectory];
        
        if (file.isDirectory) {
            
            [YLZFileHelper scanDir:file.filePath rootFile:file maxScanLevel:(maxTreeLevel <= 0 ? 0 : maxTreeLevel)];
        }
    }
...
+ (void)scanDir:(NSString *)dirPath rootFile:(YLZFile *)rootFile maxScanLevel:(NSInteger)maxLevel {
    ...
    for (NSString *relatedPath in array) {
        ...
        YLZFile *file = [YLZFileHelper cerateFileWithFilePath:fullPath isDirectory:isDirectory];
        file.level = rootFile.level + 1;
        
        if (file.isDirectory) {
            
            [YLZFileHelper scanDir:file.filePath rootFile:file maxScanLevel:maxLevel];
        }
